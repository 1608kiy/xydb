...
function handleSelectBlock (options) {
  ...
  if (view.model instanceof type.SysMLProperty || view.model instanceof type.SysMLPort) {
    app.elementPickerDialog.showDialog('选择块', view.model.type, type.SysMLBlock).then(function ({buttonId, returnValue}) {
      if (buttonId === 'ok') {
...
function handleSelectValueType (options) {
  const view = options.view
  if (view.model instanceof type.SysMLProperty || view.model instanceof type.SysMLPort) {
    app.elementPickerDialog.showDialog('选择值类型', view.model.type, type.SysMLValueType).then(function ({buttonId, returnValue}) {
      if (buttonId === 'ok') {
        app.engine.setProperty(view.model, 'type', returnValue)
      }
    })
...
function handleSelectConstraintBlock (options) {
  ...
  if (view.model instanceof type.SysMLProperty) {
    app.elementPickerDialog.showDialog('选择约束块', view.model.type, type.SysMLConstraintBlock).then(function ({buttonId, returnValue}) {
      if (buttonId === 'ok') {
        app.engine.setProperty(view.model, 'type', returnValue)
      }
    })
