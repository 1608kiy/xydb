...
class Factory extends EventEmitter {
  ...
  createDiagram(options) {
    ...
    if (!fn) {
      console.error(
        "Factory.createDiagram(): No such function of options.id: " + options.id
      );
      return null;
    }
  ...
  createModel(options) {
    ...
    if (!fn) {
      console.error(
        "Factory.createModel(): No such function of options.id: " + options.id
      );
      return null;
    }
  ...
  createModelAndView(options) {
    ...
    if (!fn) {
      console.error(
        "Factory.createModelAndView(): No such function of id: " + options.id
      );
      return null;
    }
    options = options || {};
    ...
    options.viewType =
      options.viewType ||
      this.metamodelManager.getViewTypeOf(options.modelType);
    // parasitic option
    if (options.parasitic) {
      options.containerView = options.headView;
      options.parent = options.headModel;
    }
    // container-views option
    if (Array.isArray(options["container-views"])) {
      const vs = options["container-views"];
      if (vs.some((v) => options.headView instanceof type[v])) {
        options.containerView = options.headView;
        options.parent = options.headModel;
      }
    }
    ...
    if (
      Array.isArray(options["connectable-views"]) &&
      options["connectable-views"].length > 0
    ) {
      let viewTypes = options["connectable-views"].map((dt) => {
        return type[dt];
      });
      options.tailView = this.getViewByTypes(
        options.diagram,
      ...
      options.headView = this.getViewByTypes(
        options.diagram,
        options.editor.canvas,
        options.x2,
        options.y2,
  ...
  createViewOf(options) {
    ...
    if (!fn) {
      console.error(
        "Factory.createViewOf(): No function is registered for diagram type: " +
          options.diagram.getClassName()
      );
      return null;
    }
    options = options || {};
  ...
  registerDiagramFn(id, fn) {
    if (this.diagramFn[id]) {
      console.error(
        "Factory.registerDiagramFn(): Already registered id: " + id
      );
      return;
    }
  ...
  registerModelAndViewFn(id, fn, defaultOptions) {
    if (this.modelAndViewFn[id]) {
      console.error(
        "Factory.registerModelAndViewFn(): Already registered id: " + id
      );
      return;
    }
    if (defaultOptions) {
  ...
  registerViewOfFn(diagramType, fn) {
    if (this.viewOfFn[diagramType]) {
      console.error(
        "Factory.registerViewOfFn(): Already registered diagram type: " +
          diagramType
      );
      return;
    }
  ...
  defaultDiagramFn(parent, options) {
    ...
    diagram.name = Core.Element.getNewName(
      parent[field],
  ...
  defaultModelFn(parent, field, options) {
    ...
    // Create model element;
    model = new ModelType();
    model.name = Core.Element.getNewName(
      parent[field],
      options["generate-name"] || model.getDisplayClassName()
    );
  ...
  createViewAndRelationships(editor, x, y, model, containerView, viewType) {
    ...
    if (ViewType) {
      ...
      if (
        view instanceof type.UMLGeneralNodeView &&
        diagram._parent !== model._parent
      ) {
      ...
      _.each(this.repository.getRelationshipsOf(model), (rel) => {
        ...
        if (RelViewType) {
          ...
          if (rel instanceof type.DirectedRelationship) {
            ...
            _.each(diagram.ownedViews, (v) => {
              if (
                v.model === rel.end2.reference &&
                model === rel.end1.reference
              ) {
                relView = new RelViewType();
                relView._parent = diagram;
                relView.model = rel;
                ...
                v.model === rel.end1.reference &&
                model === rel.end2.reference
              ) {
                relView = new RelViewType();
                relView._parent = diagram;
                relView.model = rel;
            ...
            // create self-links.
            if (
              rel.end1.reference === rel.end2.reference &&
              rel.end1.reference === model
            ) {
              relView = new RelViewType();
              relView._parent = diagram;
              relView.model = rel;
  ...
  defaultViewOnDiagramFn(model, diagram, options) {
    ...
    if (model instanceof type.DirectedRelationship) {
      ...
      if (directedView) {
        ...
        if (!sourceView) {
          return this.createViewAndRelationships(
            editor,
            x,
            y + 100,
            model.source
