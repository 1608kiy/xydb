...
function showFileError(err) {
  app.dialogs.showErrorDialog("文件系统错误（错误=" + err + ")");
}
...
function doFileOpen(fullPath) {
  try {
    ...
    console.log(err);
    app.dialogs.showErrorDialog(`无法打开 ${fullPath}`);
  }
}
...
async function handleExportDiagramToPNG(diagram, fullPath) {
  ...
  if (diagram) {
    ...
    app.dialogs.showAlertDialog("没有可以导出的图表。");
  }
}
...
async function handlePrintToPDF() {
  ...
  if (buttonId === "save") {
    ...
    if (diagrams.length > 0) {
      ...
      app.dialogs.showAlertDialog("没有当前图表。");
    }
  }
}
      ...
      if (views.length > 0 && diagram && diagram.canCopyViews()) {
        ...
        if (
          ...
          app.dialogs.showInfoDialog(
            "不支持复制序列图或通信图中的视图元素。\n 要复制整个序列图或通信图，请在资源管理器中复制包含该图的“交互”或者“协作(或分类器)”",
          );
        } else {
          app.dialogs.showInfoDialog(
            "没有可以复制的视图或者部分视图不能被复制。",
          );
        }
      }
    }
...
function handleRename() {
  ...
  app.dialogs.showInputDialog("输入图表名称", dgm.name).then(function ({
    buttonId,
    returnValue,
  }) {
    if (buttonId === "ok") {
...
function handleEnterLicenseKey() {
  if (app.licenseManager.getStatus() === true) {
    app.dialogs.showInfoDialog("您已经有一个有效的许可证。");
  } else {
...
function handleDeleteLicenseKey() {
  if (app.licenseManager.getStatus() === true) {
    ...
    app.dialogs.showInfoDialog("您没有有效的许可证来删除。");
